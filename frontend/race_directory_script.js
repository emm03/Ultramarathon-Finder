document.addEventListener("DOMContentLoaded", () => {
    ...
    function parseCSV(data) {
        ...
        return rows.map(row => {
            ...
            const website = columns[7]?.trim().replace(/^"|"$/g, "") || null;
            return { name, date, location, distance, continent, month, durationGroup, website };
    ...
    function displayRaces(filteredRaces) {
        ...
        racesToShow.forEach(race => {
            ...
            const linkHTML = race.website
                ? `<p><a href="${race.website}" target="_blank" class="race-link">Official Website</a></p>`
                : "";
